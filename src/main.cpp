...
// 日付: 2026.07.12 バージョン 1.8
//  - Goertzel を3ビン化 (中心 + ±341.33Hz サイドビン、003は整数Q14/006はfloat)
...
// 日付: 2026.07.20 バージョン 2.0
//  - サイド判定を min(L,H) から幾何平均 sqrt(L*H) に変更
//    低域から通過帯域へ裾を引く傾斜ノイズ(バンドノイズ)による偽符号を抑止
//  - トーンONに「中心 > max(サイドEMA, サイド瞬時)」条件を追加
//    (ESP32版 cw_decoder4 v2.0 で実機検証済みの対策を移植)
// 日付: 2026.09.18 バージョン 2.1
//  - デコード周波数を 600/700/800/900/1000Hz の5段切替に変更 (デフォルト 600Hz)
//    従来の 700/800 表示は実際には 666.7/833.3Hz だったのを表示どおりの周波数に修正
//  - 振幅しきい値の床をノイズ床連動 (x6) に変更し、入力レベル非依存化
//    (ノイズ混じりの信号を音量を絞らずにデコードできるように)
//  - しきい値の追従を立ち上がり速く(1/6)/下降遅く(1/64)に
//  - ノイズブランカを上下カウンタの積分方式に変更 (深さ 0.35 単位、2〜3 ブロック)
//    (ESP32版 cw_decoder4 v2.1/v2.2 の対策を移植)
//  - スペアナ画面のスタック溢れ (FFT バッファ破壊) を修正
//
